  ...
  describe("orgs", function () {
    describe("createOrg()", function () {
      it("errors out if bearer token is not included", function (done) {
        ...
        User.createOrg("boomer", function (err, org) {
          userMock.done();
          expect(err).to.exist();
          expect(err.message).to.equal('no bearer token included in creation of boomer');
      ...
      it("is successful if bearer token is included", function (done) {
        var userMock = nock(User.host, {reqheaders: {bearer: 'bob'} })
          .put('/org', {name: "boomer"})
          .reply(200, {
            "name": "boomer",
            "description": "",
        ...
        new (require("../../models/user"))({
          host: "https://user.com",
          bearer: "bob"
        }).createOrg("boomer", function (err, org) {
          userMock.done();
          expect(err).to.not.exist();
          expect(org.name).to.equal("boomer");
    ...
    describe("getOrg()", function () {
      it("renders an error if the org is not found", function (done) {
        var userMock = nock(User.host)
          .get('/org/notfound/user')
          .reply(404, "not found");

        User.getOrg('notfound', function (err, users) {
          userMock.done();
          expect(err).to.exist();
          expect(err.message).to.equal('org notfound not found');
          expect(users).to.be.undefined();
          done();
        });
      });

      it("returns a list of org users for orgs that exist", function (done) {
        var userMock = nock(User.host)
          .get('/org/boomer/user')
          .reply(200, {"count":1,"items":[fixtures.users.bob]});

        User.getOrg('boomer', function (err, users) {
          userMock.done();
          expect(err).to.not.exist();
          expect(users).to.exist();
          expect(users.items[0].name).to.equal(fixtures.users.bob.name);
          done();
        });
      });
    });
